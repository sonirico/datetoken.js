...
import { tokenToDate, Datetoken } from './utils';
...
describe('utils.tokenToDate', () => {
  describe('T', () => {
    it.only("in(TZ) + get(now)", () => {
      const actual = Datetoken.on('America/Chicago').get('now').toDate();
      const expected = '2018-06-18T02:39:07+00:00';
      expect(format(actual, dateFormat)).toBe(expected)
    })
  })
